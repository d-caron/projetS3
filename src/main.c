...
int main (int argc, char** argv) {
  ...
  if (strcmp(my_args.log_file, "\0") == 0) {  //  Pas de fichier de log
    ...
    logfp = fopen(my_args.log_file, "w");     // Ecriture dans le fichier
  }
  ...
  {                        
    ...
    creer_t_duel_mat(&t_duel, dim_duel);      // Création et initialisation de la matrice de duel
  ...
  if (strcmp(my_args.method, "uni1") == 0) {
    ...
    scrutin_cm(t_mat_char, t_duel, logfp);
  } else if (strcmp(my_args.method, "cp") == 0) {
    scrutin_cp(t_mat_char, t_duel, logfp);
  } else if (strcmp(my_args.method, "cs") == 0) {
    scrutin_cs(t_mat_char, t_duel, logfp);
  } else if (strcmp(my_args.method, "va") == 0) {
    scrutin_alternatif(t_mat_char, logfp);
  } else {
    if (strcmp(my_args.csv_type, "-i") == 0) {
